...
const PinnedProject = ({pinnedProjects}: Props) => (
        ...
        >
          <div
            className={classNames(
              ...
              ' bg-blue-900 flex-shrink-0 flex items-center justify-center w-16 text-white text-sm font-medium rounded-l-md',
            )}
          >
            {/* {project.initials} */}
          </div>
          <div className="flex-1 flex items-center justify-between border-t border-r border-b border-gray-200 bg-white rounded-r-md truncate">
            <div className="flex-1 px-4 py-2 text-sm truncate">
              <Link
                href={{
                  pathname: `/run_test/${project.id}`,
                  query: {title: project.title},
                }}
              >
                <a>
                  {' '}
                  <h2 className="text-gray-900 font-medium hover:text-gray-600">
                    {project.title}
                    
                  </h2>
                  <p className="text-gray-500">{project.lastUpdated}</p>
                </a>
              </Link>
            </div>
